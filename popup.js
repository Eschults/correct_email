...
function getPossibleEmails(first_name, last_name, domain) {
...
function getCorrectEmails() {
  // var first_name = $('#first_name').val();
  // var last_name = $('#last_name').val();
  // var domain = $('#domain').val();
  var correct_emails = [];
  magic(getPossibleEmails($('#first_name').val(), $('#last_name').val(), $('#domain').val()).length);
}

function magic(size) {
  if(size > -1) {
    backgroundScript(, function() {
      console.log('test');
    });
  } else {
    for (var i = 0; i < correct_emails.length; i++) {
      var p = $('<p>')
      $('#correct_emails').append(p.text(correct_emails[i])).removeClass('hidden');
    }
  }
}
